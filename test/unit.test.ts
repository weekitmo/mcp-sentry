import { extractIssueId, createStacktrace } from '../src/utils';
import { SentryIssueData } from '../src/types';
...
describe('Utils', () => {
  describe('extractIssueId', () => {
    it('should extract ID from a Sentry URL', () => {
      // 使用环境变量中的Sentry URL
      const baseUrl = process.env.SENTRY_API_BASE?.replace('/api/0/', '') || 'https://sentry.io';
      const url = `${baseUrl}/organizations/lightgame/issues/${process.env.TEST_SENTRY_ISSUE_ID || '12345'}/`;
      expect(extractIssueId(url)).toEqual(process.env.TEST_SENTRY_ISSUE_ID || '12345');
    });
